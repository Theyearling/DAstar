...
namespace my_globalplanner{
    ...
    void DAstar::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros){
        if(!initialized_){
            ...
            height = cosmap_->getSizeInCellsY();
            map_size = width * heigth;
            ...
            plan_pub_ = private_nh.advertise<nav_mags::Path>("plan", 1);
    ...
    bool DAstar::makePlan(const geometry_msgs::PoseStamped& start,
        ...
        dequeue_.push_back(Index(goal_index, 0));

        // to store the cost of the current grid to the respective starting point 
        // when searching in someone direction
        vector<float> gCost(map_size, MAX_INIT);
        vector<float> degCost(map_size, MAX_INIT);
        ...
        while(queue_.size() > 0 && dequeue_.size() > 0){
            ...
            if(degCost[i] < MAX_INIT){
                ...
                getPlan(cameFrom, decameFrom, start_index, goal_index, plan, i);
                return true;
            }

            if(i == goal_index){
                // in the positive direction, reach the ending
                getPlan(cameFrom, start_index, goal_index, plan);
                return true;
            }

            if(de_i = start_index){
                // in the opposite direction, reach the ending
                getPlan(decameFrom, start_index, goal_index, plan);
                return true;
            }
    ...
    void DAstar::getPlan(vector<int> camefrom, vector<int> decamefrom, int start_i, int goal_i
        ...
        reverse(camePath.begin(), camePath.end());
    ...
    void DAstar::addNeigbors(vector<int>& camefrom, vector<float>& gcost, int i, int goal_i, int judge){
        ...
        for(int x = -1; x <= 1; x++){
            for(int y = -1; y <= 1; y++){
                ...
                int next_x = xi + x;
                int next_y = yi + y;
                int next_i = costmap_->getIndex(next_i, xi, yi);
                ...
                if(judge == 1){
                    // in positive
                    queue_.push_back(Index(next_i, gcost[next_i] + distance * 20));
