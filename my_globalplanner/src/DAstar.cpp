    ...
    void DAstar::addNeigbors(std::vector<int>& camefrom, std::vector<float>& gcost, int i, int goal_i, int judge){
        ...
        for(int x = -1; x <= 1; x++){
            for(int y = -1; y <= 1; y++){
                ...
                // obstacle
		unsigned int n = static_cast<int>(costmap_->getCost(next_x, next_y));
std::cout<<n<<"  ";
                if(n >= 230 /*&& !(costs[next_i]==costmap_2d::NO_INFORMATION)*/) continue;

                // cost = gcost + hcost
                gcost[next_i] = gcost[i] + getGcost(i, next_i);
		//gcost[next_i] = getPre_min(gcost, next_i, camefrom);
    ...
    float DAstar::getPre_min(std::vector<float> gcost, int i, std::vector<int>& camefrom){
        float gcost_min = gcost[i];
        int res = i;
        unsigned x_i, y_i;
		...
		float gc = gcost[next_i] + getGcost(i, next_i);
                if(gc < gcost_min){
                    gcost_min = gc;
                    res = next_i;
                }
    ...
    double DAstar::getHcost(int next_i, int goal_i){
        ...
        costmap_->indexToCells(goal_i, tmp1, tmp2);
